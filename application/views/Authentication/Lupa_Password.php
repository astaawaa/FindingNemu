    ...
    <div class="container d-flex justify-content-center align-items-center" style="height:100vh;">
        <div class="container-form1">
            ...
            <form action="<?= base_url('Cauth/lupapassword') ?>" method="post" class="d-flex flex-column justify-content-between" style="height:220px;">
                <div class="py-4">
                    <div class="mb-3">
                        <input type="email" placeholder="Email" class="inputan fs-6 rounded p-2" name="email">
                        <?= form_error('email','<small class="text-danger" pl-3>','</small>');?>
                    </div>
                </div>
                <div>
                <button type="submit" class="button background-blue p-2 border-none w-100 rounded text-light" id="buttonSub">DAPATKAN KODE</button>
                <button type="button" class="button background-blue p-2 border-none w-100 rounded text-light" id="btnLoad">
                    <div class="spinner-border" role="status"></div>
                </button>
                    <a href="<?=base_url("Cauth/login")?>" class="button background-gray p-1 mt-2 rounded text-light" >KEMBALI</a>
                </div>
            </form>
        </div>
    </div>
